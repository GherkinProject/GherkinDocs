...
class MyForm(QtGui.QMainWindow):
    ...
    #print self.server.system.listMethods()
        ...
        for u in self.songs:
            try:    
                self.ui.addTrack(u['title'],u['location'])
            except:
                self.ui.addTrack("Unknown",u['location']) 
# l'except est present pour les fichiers n'ayant pas de titre.
    ...
    #loading song into the server
        ...
        QtCore.QObject.connect(self.ui.AudioTrack, QtCore.SIGNAL("itemDoubleClicked(QTreeWidgetItem*,int)"), self.ui.iconChange )
